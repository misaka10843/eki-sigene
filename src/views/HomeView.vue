...
function deriveStripColor(hex: string): string {
  const [h, s, l] = hexToHsl(hex)
  return hslToHex(h, s, l < 50 ? Math.min(100, l + 25) : Math.max(0, l - 25))
}

/* ── Track last auto-derived values to detect manual overrides ── */
const _auto = reactive({ leftLine: '', rightLine: '', numbering: '' })

watch(() => state.routeColors[0], (color) => {
  if (!color) return
  const strip = deriveStripColor(color)

  const ls = state.leftStations[0]
  if (ls && (!_auto.leftLine || ls.lineColor === _auto.leftLine)) {
    ls.lineColor = strip; _auto.leftLine = strip
  }

  const rs = state.rightStations[0]
  if (rs && (!_auto.rightLine || rs.lineColor === _auto.rightLine)) {
    rs.lineColor = strip; _auto.rightLine = strip
  }

  const n = state.sta.numberings[0]
  if (n && (!_auto.numbering || n.color === _auto.numbering)) {
    n.color = color; _auto.numbering = color
  }
})
